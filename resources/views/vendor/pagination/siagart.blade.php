@if ($paginator->hasPages())
    <nav role="navigation" aria-label="{{ __('Pagination Navigation') }}" class="flex items-center justify-between">
        ...
        {{-- Tampilan Mobile (Simpel Prev/Next) --}}
                        ...
                        @if (is_array($element))
                            @foreach ($element as $page => $url)
                                @if ($page == $paginator->currentPage())
                                    {{-- ACTIVE STATE (Ganti warna disini, saya pake Amber) --}}
                                    <span aria-current="page">
                                        <span class="relative inline-flex items-center px-4 py-2 -ml-px text-sm font-bold text-white bg-amber-500 border border-amber-500 cursor-default leading-5">{{ $page }}</span>
                                    </span>
                                @else
