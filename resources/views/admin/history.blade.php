...
@section('content')
    ...
    <div x-data="{ showFilters: {{ request()->anyFilled(['search', 'category', 'date_start', 'date_end']) ? 'true' : 'false' }} }" class="flex flex-col h-full w-full">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full overflow-hidden w-full">
            ...
            <!-- HEADER SECTION -->
            <div
                class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-center bg-gray-50/50 gap-4 flex-shrink-0">
                ...
                @if (request()->anyFilled(['search', 'category', 'date_start', 'date_end']))
                    <a href="{{ route('admin.laporan.history') }}"
                        class="inline-flex items-center px-3 py-1.5 text-xs font-medium text-red-600 bg-red-50 rounded-lg hover:bg-red-100 transition-colors border border-red-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                        ...
                        <div class="md:col-span-3">
                            ...
                            <select name="category" id="category"
                                class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-white">
                                <option value="">Semua Kategori</option>
                                <option value="Kebakaran" {{ request('category') == 'Kebakaran' ? 'selected' : '' }}>
                                    Kebakaran</option>
                                <option value="Sampah" {{ request('category') == 'Sampah' ? 'selected' : '' }}>Sampah
                                </option>
                                <option value="Infrastruktur"
                                    {{ request('category') == 'Infrastruktur' ? 'selected' : '' }}>Infrastruktur</option>
                                <option value="Keamanan" {{ request('category') == 'Keamanan' ? 'selected' : '' }}>Keamanan
                                </option>
                                <option value="Darurat" {{ request('category') == 'Darurat' ? 'selected' : '' }}>Darurat
                                </option>
                            </select>
                        </div>
                        ...
                        <div class="md:col-span-4 grid grid-cols-2 gap-2">
                            <div>
                                <label for="date_start" class="block text-xs font-medium text-gray-500 mb-1">Dari
                                    Tanggal</label>
                        ...
                        <div class="md:col-span-1 flex items-end">
                            <button type="submit"
                                class="w-full bg-slate-800 hover:bg-slate-900 text-white font-medium py-2 px-4 rounded-lg text-sm transition shadow-sm h-[38px] flex justify-center items-center">
                                Cari
                            </button>
                        </div>
                    </div>
            ...
            <div class="flex-1 overflow-auto bg-white relative w-full">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    <thead
                        class="bg-gray-50 text-gray-500 font-medium uppercase tracking-wider sticky top-0 z-10 shadow-sm">
                        <tr>
                        ...
                        @forelse($laporan as $item)
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 text-gray-500 font-mono">
                                    #LP-{{ $laporan->firstItem() + $loop->index }}
                                </td>
                                <td class="px-6 py-4 text-gray-500">{{ $item->created_at->format('d M Y, H:i') }}</td>
                                <td class="px-6 py-4 font-medium">{{ $item->name }}</td>
                                <td class="px-6 py-4">
                                    <span
                                        class="bg-gray-100 text-gray-600 py-1 px-2.5 rounded text-xs border border-gray-200 font-medium">
                                        {{ $item->category }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="inline-flex items-center gap-1.5 rounded-full bg-{{ $statusColor }}-50 px-2.5 py-1 text-xs font-semibold text-{{ $statusColor }}-600 border border-{{ $statusColor }}-100">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ route('admin.laporan.show', $item->id) }}"
                                        class="text-blue-600 hover:text-blue-700 font-medium text-xs border border-blue-200 px-3 py-1.5 rounded hover:bg-blue-50 transition">
                                        Detail
                                    </a>
                                </td>
                            </tr>
