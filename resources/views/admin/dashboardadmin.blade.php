    ...
    <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6">
        ...
        <div class="w-full bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
                ...
                <a href="{{ route('admin.laporan.history') }}" class="text-sm text-amber-600 font-medium hover:text-amber-700">Lihat Semua</a>
            </div>
            <div class="overflow-x-auto">
                <table class="w-full text-left text-sm whitespace-nowrap">
                    ...
                    <tbody class="divide-y divide-gray-100 text-slate-700">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="flex items-center gap-1">
                                        <span class="w-2 h-2 rounded-full bg-{{ $catColor }}-500"></span> {{ $item->category }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="inline-flex items-center gap-1 rounded-full bg-{{ $statusColor }}-50 px-2.5 py-1 text-xs font-semibold text-{{ $statusColor }}-600 border border-{{ $statusColor }}-100">
                                        <span class="h-1.5 w-1.5 rounded-full bg-{{ $statusColor }}-500"></span> {{ $statusLabel }}
                                    </span>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="5" class="text-center py-8 text-gray-400">Belum ada laporan.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
