...
<div class="flex flex-1 flex-col overflow-hidden h-screen bg-gray-50">
    ...
    <header class="h-16 flex items-center bg-white px-6 shadow-sm border-b">
        <h2 class="text-xl font-bold text-slate-800">Detail Laporan dari 
            <span class="text-red-500">{{ $report->name }}</span></h2>
    </header>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    @php
                        // PERBAIKAN: Gunakan 'status_report' (sesuai DB kamu)
                        // Dan logika fallback yang lebih aman
                    ...
                    <div class="mb-6 p-4 rounded-lg bg-gray-50 border border-gray-200">
                        ...
                        @if($currentStatus == \App\Enums\StatusLaporan::MENUNGGU->value)
                            <span class="text-amber-600 font-bold flex items-center gap-2">
                                <span class="w-3 h-3 rounded-full bg-amber-500"></span> {{ $statusEnum->label() }}
                            </span>
                        @elseif($currentStatus == \App\Enums\StatusLaporan::PROSES->value)
                            <span class="text-blue-600 font-bold flex items-center gap-2">
                                <span class="w-3 h-3 rounded-full bg-blue-500"></span> {{ $statusEnum->label() }}
                            </span>
                        @elseif($currentStatus == \App\Enums\StatusLaporan::SELESAI->value)
                            <span class="text-green-600 font-bold flex items-center gap-2">
                                <span class="w-3 h-3 rounded-full bg-green-500"></span> {{ $statusEnum->label() }}
                            </span>
                        @elseif($currentStatus == \App\Enums\StatusLaporan::DITOLAK->value)
                            <span class="text-red-600 font-bold flex items-center gap-2">
                                <span class="w-3 h-3 rounded-full bg-red-500"></span> {{ $statusEnum->label() }}
                            </span>
                        @endif
                    </div>

                    <!-- Form Update Status -->
                    <form action="{{ route('admin.laporan.updateStatus', $report->id) }}" method="POST" class="space-y-3">
                        @csrf
                        
                        <!-- SKENARIO 1: Status MENUNGGU (Tombol Tolak & Verifikasi) -->
                        @if($currentStatus == \App\Enums\StatusLaporan::MENUNGGU->value)
                            <div class="grid grid-cols-2 gap-3">
                                <!-- Tombol TOLAK (Merah) -->
                                <button type="submit" name="status" value="{{ \App\Enums\StatusLaporan::DITOLAK->value }}" onclick="return confirm('Apakah Anda yakin ingin menolak laporan ini?')" class="w-full py-3 px-4 bg-white border-2 border-red-500 text-red-600 hover:bg-red-50 font-medium rounded-lg transition flex items-center justify-center gap-2">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                    Tolak
                                </button>

                                <!-- Tombol VERIFIKASI (Biru) -->
                                <button type="submit" name="status" value="{{ \App\Enums\StatusLaporan::PROSES->value }}" class="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition flex items-center justify-center gap-2">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                    Verifikasi (Valid)
                                </button>
                            </div>
                            <p class="text-xs text-gray-400 text-center mt-2">
                                *Laporan ditolak akan dihapus otomatis dalam 1 bulan.
                            </p>
                        @endif

                        <!-- SKENARIO 2: Status PROSES (Tombol Selesai) -->
                        @if($currentStatus == \App\Enums\StatusLaporan::PROSES->value)
                            <button type="submit" name="status" value="{{ \App\Enums\StatusLaporan::SELESAI->value }}" class="w-full py-3 px-4 bg-green-600 hover:bg-green-700 text-white font-medium rounded-lg transition flex items-center justify-center gap-2">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                Tandai Selesai
                            </button>
                        @endif

                        <!-- SKENARIO 3: Status SELESAI / DITOLAK (Info Saja) -->
                        @if($currentStatus == \App\Enums\StatusLaporan::SELESAI->value)
                            <div class="text-center text-sm text-green-600 py-3 bg-green-50 border border-green-200 rounded-lg font-medium">
                                ✓ Laporan telah diselesaikan.
                            </div>
                        @endif

                        @if($currentStatus == \App\Enums\StatusLaporan::DITOLAK->value)
                            <div class="text-center text-sm text-red-600 py-3 bg-red-50 border border-red-200 rounded-lg font-medium">
                                ✕ Laporan ini ditolak (Tidak Valid).
                            </div>
                        @endif
                    </form>
                </div>
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div class="flex items-center gap-4 mb-4">
                        <div class="h-12 w-12 rounded-full bg-slate-200 flex items-center justify-center text-lg font-bold text-slate-600 uppercase">
                    ...
                    @if($report->wa_number)
                        <a href="https://wa.me/{{ $report->wa_number }}" target="_blank" class="w-full py-2.5 px-4 bg-green-50 text-green-700 hover:bg-green-100 font-medium rounded-lg transition flex items-center justify-center gap-2 border border-green-200">
